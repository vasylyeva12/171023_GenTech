...
select count(country_id) as countr_count
from countries;
...
-- Вывести сотрудников с дольжностью(job_title) Programmer,  которые получают больше средней зарплаты всех сотрудников.
...
-- Вывести сотрудников(first_name, last_name, salary) с дольжностью(job_title) Programmer,  которые получают больше средней 
